...
class Edit extends React.Component {
  ...
  componentWillReceiveProps(nextPropsFromRedux) {
    if (nextPropsFromRedux.Agent.record_data) {
      this.setState({
        shiftname: nextPropsFromRedux.Agent.record_data.data.shift_name,
        usergroup: nextPropsFromRedux.Agent.record_data.data.user_group
          ? nextPropsFromRedux.Agent.record_data.data.user_group
          : "",
        totaltime: nextPropsFromRedux.Agent.record_data.data.totaltime
          ? nextPropsFromRedux.Agent.record_data.data.shift_length
          : "",
        reportoption: nextPropsFromRedux.Agent.record_data.data.reportoption
          ? nextPropsFromRedux.Agent.record_data.data.reportoption
          : "",
        shiftstime: nextPropsFromRedux.Agent.record_data.data.shiftstime
          ? nextPropsFromRedux.Agent.record_data.data.shiftstime
          : "",
        shiftetime: nextPropsFromRedux.Agent.record_data.data.shiftetime
          ? nextPropsFromRedux.Agent.record_data.data.shift_start_time +
            nextPropsFromRedux.Agent.record_data.data.shift_length
          : "",
        week_checked: nextPropsFromRedux.Agent.record_data.data.shift_array
          ? JSON.parse(nextPropsFromRedux.Agent.record_data.data.shift_array)
          : week_days
      });

      console.log("++++++++++++11");
      console.log(nextPropsFromRedux.Agent.record_data.data.shift_array);
      console.log("++++++++++++66666666666666666666");

      /*              let exiting_str =[];
                 exiting_str = this.state.week_checked;
                 let str = nextPropsFromRedux.Agent.record_data.data.shift_weekdays;


                for (var i = 0; i < exiting_str.length; i++) {

                      //console.log(exiting_str[i]);
                      if(str.includes(exiting_str[i].day)){


                          exiting_str[i].checked=true;
                      }
                      //console.log(exiting_str[i].day);

                  }*/

      // this.setState({week_checked:exiting_str});

      /*
                let array=nextPropsFromRedux.Agent.record_data.data.shift_weekdays;

              for (var i = 0; i < array.length; i++) {

                      alert(array.charAt(i));
                      this.state.week_checked[i]
  ...
  Calculationhandler = e => {
    ...
    while (st_value.length < 4) {
      st_value = "0" + st_value;
    }
    while (et_value.length < 4) {
      et_value = "0" + et_value;
    }
    ...
    if (start_time.value == end_time.value) {
      ...
      if (shift_hour < 10) {
        shift_hour = "0" + shift_hour;
      }
      if (shift_minute < 10) {
        shift_minute = "0" + shift_minute;
      }
      var shift_length = shift_hour + ":" + shift_minute;
    }

    this.setState({
      shiftstime: parseInt(start_time.value),
      shiftetime: parseInt(end_time.value),
      totaltime: shift_length
    });
  };
  ...
  handleSubmit = () => {
    ...
    let error = false;
    shiftname == ""
      ? (this.setState({ shiftnameError: true }), (error = true))
      : this.setState({ shiftnameError: false });
    shiftId == ""
      ? (this.setState({ shiftIdError: true }), (error = true))
      : this.setState({ shiftIdError: false });
      ...
      return (
        <div>
          ...
          >
            {/* {showAlert &&
            alertTitle === "Success" &&
            createNotification(SUCCESS, SUCCESS, alertTitle)}
          {showAlert &&
            alertTitle === "Error" &&
            createNotification(ERROR, ERROR, alertTitle)}
            {" "} */}
            {alertContent}{" "}
          </SweetAlert>
          <div className="row">
              ...
              <form className="row" noValidate autoComplete="off">
                ...
                <div className="col-md-12">
                  <div className="form-group">
                    <TextField
                      id="agent-script-user-group-native"
                      select
                      label="User Group"
                ...
                >
                  ...
                  <div>
                    ...
                    <a
                      ...
                      onClick={this.Calculationhandler}
                    >
                ...
                >
                  <div className="form-group" style={{ marginTop: "20px" }}>
                    <label htmlFor="shift_weekdays">
                      {" "}
                      <h3>Shift Weekdays </h3>
                    </label>
                    {week_checked.map((row, index) => {
                      return (
                        <div key={week_checked[index].day}>
                          {" "}
                          <Checkbox
                            color="primary"
                            onClick={e =>
                            ...
                            value={week_checked[index].day}
                            defaultChecked={week_checked[index].status}
                          />{" "}
                          {week_checked[index].name}
                        </div>
                      );
                    })}
                  </div>

...
import { API_CLONE_INBOUND_NUMBER_DATA } from "../../../../../../constants";
...
class Clone extends React.Component {
  ...
  cloneAgentGroup = () => {
    ...
    let formdata = {
      new_id: this.state.list_id,
      from_id: this.state.source_agent_group
    };

    axios
      .post(API_CLONE_INBOUND_NUMBER_DATA, formdata)
      .then(response => {
        console.log(response);
        $this.setState({
          sucsess_msg: "DID cloned successfully",
          inprocess: false
        });
      })
  ...
  componentWillReceiveProps(nextPropsFromRedux) {
    this.setState({
      dids_group: nextPropsFromRedux.Global.clone_did_ids
        ? nextPropsFromRedux.Global.clone_did_ids
        : ""
    });
  }
  ...
  componentDidMount() {
    this.props.fetchGlobal(["clone_did_ids"]);
      ...
      <div className="w-100">
        ...
        <div>
          {this.allStepsCompleted() ? (
            ...
            <div>
              {activeStep == 0 ? (
                <CardBox styleName="col-lg-12" heading="DID ">
                  <form className="row" noValidate autoComplete="off">
                    <div className="col-lg-12 col-sm-12 col-12">
                      <FormControl className="w-100 mb-2">
                        ...
                        >
                          {dids_group &&
                            dids_group.map((option, i) => (
                              <MenuItem key={i} value={option.did_pattern}>
                                {option.did_pattern}
                              </MenuItem>
                            ))}
                        </Select>
                        <FormHelperText>
                          Your new did will be cloned from this did.
                        </FormHelperText>
                      </FormControl>
                    </div>
                    <div className="col-md-12 col-12">
                      <FormControl className="w-100 mb-2" error>
                        <TextField
                          ...
                          helperText="This is your new DID ID.."
                          error={listError}
                        />
                      </FormControl>
                    </div>
                    ...
                    <div className="col-md-12 col-12">
                      <FormControl className="w-100 mb-2">
                        <TextField
                          ...
                          value={list_id}
                          disabled
                          helperText="This is your new New DID ID."
                          error={false}
                        />
                      </FormControl>
                    </div>
        ...
        {(this.props.Global.isLoading || this.state.isLoding) && (
          <div className="loader-view" id="loader-view">
            <CircularProgress />
          </div>
        )}

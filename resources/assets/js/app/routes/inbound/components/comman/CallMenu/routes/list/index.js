...
class List extends React.Component {
  ...
  handleEditEventHandler = id => {
    this.props.history.push("/app/inbound/callmenu/edit/" + id);
  };
  ...
  getAllData = e => {
    ...
    axios
      .get(
        "/api/inbound-lists/callmenus?limit=5&&page=1&&group_id=" +
  ...
  render() {
    ...
    const pageTitleTEXT = "Call Menus Using This In-Group : " + pageTitle;
                ...
                <TableBody>
                  {this.state.records.length != 0 &&
                    this.state.records.map((row, index) => {
                      return (
                        <TableRow key={index}>
                          <TableCell style={divStyle}>{row.menu_id}</TableCell>
                          ...
                          <TableCell style={divStyle}>
                            <center>
                              <Tooltip title="Modify Agent">
                                <IconButton
                                  onClick={() =>
                                    this.handleEditEventHandler(row.menu_id)
                                  }
                                >

...
class List extends React.Component {
  ...
  handleChangeRowsPerPage = event => {
    this.setState({ rowsPerPage: event.target.value });
    this.fetchAllListFpg(this.state.page, event.target.value);
  };
  ...
  DeleteConfirmHandler = () => {
    ...
    let URl = "/api/delete-filter-phone-group";
    _this.setState({ isLoding: true });
    axios
      .get(URl + "?filter_phone_group_id=" + this.state.deleteInboundId)
      .then(response => {
                ...
                <TableBody>
                  {ajaxdata.length != 0 &&
                    ajaxdata.map((n, i) => {
                      return (
                        <TableRow key={i}>
                          <TableCell>{n.filter_phone_group_id}</TableCell>
                          <TableCell>{n.filter_phone_group_name}</TableCell>
                          <TableCell>
                            {n.filter_phone_group_description}
                          </TableCell>
                          ...
                          <TableCell>
                            <Tooltip title="Download">
                              <IconButton
                                onClick={() =>
                                  this.downloadCsv(n.filter_phone_group_id)
                                }
                              >
                                <i
                          ...
                          <TableCell className="text-center">
                            <Tooltip title="Modify Campaign Status ">
                              <IconButton>
                                <a
                                  className="teal-text"
                                  onClick={() =>
                                    this.handleAddEvent(n.filter_phone_group_id)
                                  }
                                >
                            ...
                            <Tooltip title="Delete Campaign">
                              <IconButton
                                onClick={() =>
                                  this.DeleteInboundHandler(
                                    n.filter_phone_group_id
                                  )
                                }
                              >

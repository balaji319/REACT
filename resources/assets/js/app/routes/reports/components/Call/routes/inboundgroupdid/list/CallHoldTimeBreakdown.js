...
class CallHoldTimeBreakdown extends React.Component {
    constructor(props) {
        ...
        this.state ={
            graph_data : [],
            HoldTimeBreakDown : this.props.mydata.data.HoldTimeBreakDown
        }
    }    
    ...
    render() {
        const {graph_data, HoldTimeBreakDown} = this.state;
        const {mydata, display_as} = this.props;
        
        for (var i = 0; i < 61;) {
            var d = 'hd_'+i;
            var data = { "name" : i, "value" : this.props.mydata.data.HoldTimeBreakDown[d]}
            this.state.graph_data.push(data);   
            i=i+5;
        }
        var data = { "name" : 90, "value" : this.props.mydata.data.HoldTimeBreakDown.hd_90}
        this.state.graph_data.push(data);   
        var data = { "name" : '99+', "value" : this.props.mydata.data.HoldTimeBreakDown.hd_99}
        this.state.graph_data.push(data);
                    ...
                    <div className="table-responsive-material">
                        <Table>
                            ...
                            <TableBody>
                                <TableRow>
                                { 
                                    Object.keys(mydata.data.HoldTimeBreakDown).map((item, i) =>
                                        <TableCell key={i}>{mydata.data.HoldTimeBreakDown[item]}</TableCell>
                                    ) 
                                }
                                </TableRow>
                            </TableBody>

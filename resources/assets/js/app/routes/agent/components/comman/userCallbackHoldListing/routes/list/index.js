...
class List1 extends React.Component {
        ...
        handleChangeRowsPerPage = event => {this.setState({rowsPerPage: event.target.value}); };
            ...
            moveEventHandler =() =>{
                    ...
                    (array_selected !='' && select_type !='' ) ? this.moveData(array_selected,select_type,agent_selected) : createNotification("Error","Error","Please select check box to delete a record.")
           ...
           moveData = (ids,select_type,agent_selected) => {
                        ...
                        let formData ={
                         ...
                         axios.post('/api/move-call-back',formData)
                                  .then(response => {
                ...
                deleteEventHandler =() =>{
                let array_selected  = this.state.selected;
                  array_selected !='' ? this.deleteData(array_selected) : createNotification("Error","Error","Please select check box to delete a record.")
              ...
              deleteData = (ids) => {
                ...
                let formData ={
                 ...
                 this.setState({isLoading: true});
                 axios.post('/api/delete-call-back',formData)
                          .then(response => {
                        createNotification("Success","Success",response.data.message)
                        this.getAllData();
                        this.setState({isLoading: false})
            ...
            changeEventHandler =(e) =>{   e.target.value == 'USERONLY' ? this.setState({move_to_user:true,select_type:e.target.value}): this.setState({move_to_user:false,select_type:e.target.value})}

            changeAgentEventHandler =(e) =>{  this.setState({agent_selected:e.target.value}) }
        ...
        deleteDataByType = (formData) => {
                 ...
                 this.setState({isLoading: true});
                 axios.post('/api/delete-month-call-backs',formData)
                          .then(response => {
                        createNotification("Success","Success",response.data.message)
                        this.getAllData();
                        this.setState({isLoading: false})
            ...
            componentWillReceiveProps(nextPropsFromRedux) {
              this.setState({
                ...
                agents:(nextPropsFromRedux.Global.agent_c ) ? nextPropsFromRedux.Global.agent_c : ""
              });
            ...
            componentDidMount() {
             ...
             getAllData = (id) => {
                ...
                let search ='';
                 this.setState({isLoading: true});
                 axios.get('/api/call-back-list?page=' + current_page + '&page_size=' + rowsPerPage +'&search=' + search + '&user=' + this.state.pageTitle)
                        .then(response => {
                        this.setState({records: response.data.data.data});
                        this.setState({isLoading: false})
                    })
                    .catch(function(error) {
                        console.log(error);
            ...
            render() {
                const {data, order, orderBy, selected,select_type , rowsPerPage,records, agents,page, move_to_user ,showAlert,alertContent,alertTitle} = this.state;
            ...
            const divStyle = {padding: '4px 30px 4px 24px'};
                return (
                        <div>
                            <ContainerHeader match={this.props.match} title='Call Menus Using This In-Group :' />
                                ...
                                <Toolbar
                                    ...
                                    <div className="title">
                                          ...
                                          <div className="col-lg-12   col-left" >
                                             <div className="dataTables_length" id="table-1_length" style={{display:'inline-flex'}}>
                                                <div className="col-lg-4">
                                                    <a href="#" href="javascript:void(0)"  onClick={this.deleteEventHandler} id="deletebutton" className="btn btn-success">Delete Callbacks</a>
                                                </div>
                                                  <div className="col-lg-4">
                                                    <select name="selectdelete" id="selectdelete" onChange={this.changeEventHandler} className="form-control"><option value>Select</option><option value="ANYONE">Move Selected Callbacks to Anyone</option>
                                                    <option value="USERONLY">Move Selected Callbacks to User Only</option>
                                                 </select>
                                                </div>
                                                {move_to_user &&
                                                     <div className="col-lg-4">
                                                    <select name="user" id="user"  onChange={this.changeAgentEventHandler}  className="form-control">
                                                    <option key='' value="" > Select Agent </option>
                                                    {agents.map(option => (
                                                        <option key={option.user} value={option.user}> {option.user} - {option.full_name}</option>
                                                    ))}

                                                 </select>
                                             </div>}
                                                  <div className="col-lg-4">
                                                   <a href="javascript:void(0)"  id="deletebutton" onClick={this.moveEventHandler} className="btn btn-success"> Move Callbacks</a>
                                                </div>
                                            ...
                                            <TableBody>
                                                ...
                                                {records.map(n => {
                                                    ...
                                                    return (
                                                            ...
                                                            <TableRow
                                                                hover
                                                                onClick={event => this.handleClick(event, n.callback_id)}
                                                                onKeyDown={event => this.handleKeyDown(event, n.callback_id)}
                                                                role="checkbox"
                                                           ...
                                                           <TableCell style={ divStyle }>
                                                             ...
                                                             <TableCell style={ divStyle }>{n.callback_time}</TableCell>
                                                ...
                                                {  records.length == 0 && this.state.isLoading  ? <TableRow><TableCell colSpan="11" ><center>Loading ......... </center></TableCell></TableRow> : records.length == 0  ?  <TableRow><TableCell colSpan="11"><center>No Records Found </center></TableCell></TableRow> :<TableRow><TableCell colSpan="11"><center> </center></TableCell></TableRow> }
                                            </TableBody>
                                            <TableFooter>
                                                <TableRow>
                                                    <TablePagination
